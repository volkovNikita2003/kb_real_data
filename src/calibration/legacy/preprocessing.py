...
cam_shift_um = calc_matrix_shift_m(center_cam)*1e6
...
plt.vlines([x_fit_orig[cut]], min(y_fit_orig), max(y_fit_orig)*1.2, 'black', '--', label=f"cut, x={x_fit_orig[cut]:4.0f} um")
...
diag_matrix_img_mm = pix_img_width_m_cor * np.sqrt(2592**2 + 1944**2) * 1e3
...
if is_add_lin:
    ...
    pix_1_diff_min = np.argmin(signal_lin[800:1000]) + 800
    ...
    plt.vlines([x_fit_orig[cut]], min(y_fit_orig), max(y_fit_orig)*1.05, 'black', '--', label=f"cut, x={x_fit_orig[cut]:4.0f} um")
...
f"количество_пикселей_длина_(шт): 2592\n"
f"количество_пикселей_ширина_(шт): 1944\n"

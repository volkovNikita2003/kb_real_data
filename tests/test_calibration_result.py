...
def camera() -> CameraCalibrationResult:
    return CameraCalibrationResult(6.19, 2592, 1944, 1.7e-4, 4.9e-5, 1.91e-6)


def line() -> LineSensorCalibrationResult:
    return LineSensorCalibrationResult(0.88, 5.09, 7.5, 8.1e-6, 2.02e-4, 0.31, 0.02, 1752.0)
...
class CalibrationResultTests(unittest.TestCase):
    def test_round_trip_with_line_sensor(self) -> None:
        ...
        self.assertEqual(expected["camera"]["width_px"], 2592)
    ...
    def test_invalid_geometry_and_dimensions_are_rejected(self) -> None:
        with self.assertRaisesRegex(CalibrationResultError, "positive"):
            CameraCalibrationResult(6.19, 0, 1944, 0.0, 0.0, 1e-6)
        with self.assertRaisesRegex(CalibrationResultError, "less than"):
            LineSensorCalibrationResult(5.0, 1.0, 7.5, 1e-6, 1e-6, 1.0, 0.0, 0.0)

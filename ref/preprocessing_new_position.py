...
# Canonical machine-readable reference values.  The legacy output above is
# intended for a human and its formatting is inconvenient for regression
# tests, so retain the final floating-point values at full text precision.
calibration_reference = {
    "camera_matrix_diagonal_mm": diag_matrix_img_mm,
    "camera_width_px": cfg.W,
    "camera_height_px": cfg.H,
    "camera_x_shift_m": cam_shift_um_cor[0] * 1e-6,
    "camera_y_shift_m": cam_shift_um_cor[1] * 1e-6,
    "camera_pixel_width_m": pix_img_width_m_cor,
}
if is_add_lin:
    calibration_reference.update({
        "line_start_angle_deg": np.degrees(phi_left_rad),
        "line_end_angle_deg": np.degrees(phi_right_rad),
        "line_logarithmic_radius_percent": 7.5,
        "line_pixel_width_m": width_pix_x_m_cor,
        "line_pixel_height_m": width_pix_y_m_cor,
        "line_to_camera_coefficient": coef_lin_to_cam,
        "line_shift_m": shift_lin_m_cor,
        "line_peak_pixel": pix_max_ampl_cor,
    })

with (PATH_SAVE_PREPROCESSING_DIR / "calibration_reference.txt").open(
    "w", encoding="utf-8"
) as stream:
    for name, value in calibration_reference.items():
        stream.write(f"{name}\t{value:.17g}\n")

np.savetxt(
    PATH_SAVE_PREPROCESSING_DIR / "camera_calibration_signal.txt",
    np.column_stack((x_distance_um_cam_cor, signal_norm_cam, cam_fit_2)),
    delimiter="\t",
...
if is_add_lin:
    np.savetxt(
        PATH_SAVE_PREPROCESSING_DIR / "line_calibration_signal.txt",
        np.column_stack((x_distance_um_lin_cor, signal_norm_lin, lin_fit)),
        delimiter="\t",
        header="distance_um\tnormalized_signal\tfitted_signal",
        comments="",
        fmt="%.17g",
